...
/// HUD purely as reassurance that Murmur is actually hearing you — not a
/// second recognition path. The real transcript still comes from whichever
/// engine `Settings.engine`/`Settings.parakeetModel` actually selects, run
...
/// recording (see [[project_murmur_nemotron_and_flash_engines]]). That bug
