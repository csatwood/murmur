...
#    Rust dependencies (linked into the same Murmur binary) both bring in
...
(cd "$FFI_DIR" && cargo test --release --locked)
(cd "$FFI_DIR" && cargo build --release --locked --target aarch64-apple-darwin)
...
echo "==> Verifying Murmur still links"
